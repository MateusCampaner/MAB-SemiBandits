...
def calcular_medias_bracos(matriz_bracos, num_bracos, num_rodadas):
    ...
    for i in range(num_bracos):
        ...
        for j in range(num_rodadas):
            braço_escolhido = random.randint(matriz_bracos[i][0], matriz_bracos[i][2])
            numero_aleatorio = random.randint(braço_escolhido, braço_escolhido)
            valores_gerados.append(numero_aleatorio)
        media_braco = sum(valores_gerados) / len(valores_gerados)
...
taxa_exploracao = st.sidebar.slider("Taxa de exploração (%):", min_value=0, max_value=100, value=50)
